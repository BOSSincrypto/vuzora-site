  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Политика конфиденциальности – Vuzora" },
      {
        ...
        content:
          "Какие персональные данные собирает Vuzora и как с ними обращается.",
      },
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "Конфиденциальность", item: abs("/legal/privacy") },
          ],
        }),
      },
    ],
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-20 md:px-10 md:py-28 text-white/85">
      ...
      <Section title="1. Оператор данных">
        <p>
          {BRAND.legal.entity}, ИНН {BRAND.legal.inn}, {BRAND.legal.city}. Связь
      ...
      <Section title="2. Какие данные мы обрабатываем">
        <ul className="list-disc space-y-1 pl-5">
          <li>Telegram-идентификатор и имя аккаунта (передаются Telegram API при первом запуске бота).</li>
        ...
        <p className="mt-3">
          Мы не собираем e-mail, номер телефона, фамилию, паспортные данные,
          геолокацию и платёжные реквизиты. Оплата подписки проходит через
          внешнего платёжного провайдера – см. его политику.
        </p>
      </Section>
      ...
      <Section title="3. Цели обработки">
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>Поддержка пользователя в боте{" "}
      ...
      <Section title="4. Сроки и хранение">
        <p>
          Данные хранятся, пока активна подписка либо до момента, когда
          пользователь явно удалит аккаунт командой /delete в боте. После
          удаления данные стираются в течение 30 календарных дней, кроме
          обезличенных агрегированных логов.
        </p>
      </Section>
      ...
      <Section title="5. Третьи стороны">
        ...
        <p className="mt-3">
          Vuzora не передаёт данные рекламным сетям и не использует сторонние
          трекеры на сайте: только обезличенная веб-аналитика без cookie.
        </p>
      </Section>

      <Section title="6. Права пользователя">
        <p>
          Пользователь вправе получить копию своих данных, исправить их или
          удалить, отправив запрос на{" "}
          <a className="underline" href={`mailto:${BRAND.email}`}>
            {BRAND.email}
          </a>
