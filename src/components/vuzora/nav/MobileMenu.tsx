 ...
 *
 * The closed panel stays in the initial HTML (absolute + `hidden` + `inert`)
 * so crawlable Telegram `data-cta` anchors and menu destinations remain
 * present for no-JS / static release validation without inflating header height.
 *
...
export const MobileMenu = forwardRef<HTMLDivElement, Props>(function MobileMenu(
  ...
  // Always render destinations/CTAs. Closed state uses display + inert so the
  // absolute panel never inflates nav height and is removed from tab/hit testing,
  // while static/no-JS HTML still exposes the anchors for release validation.
  return (
    <div
      id={id}
      ref={ref}
