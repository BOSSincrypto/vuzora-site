 ...
 *
 * Drop the trailing slash, append `.md` — the convention agents probe, so
 * `/pricing/` mirrors at `/pricing.md` and `/unis/msu/` at `/unis/msu.md`.
 * The root has no name to drop and takes `/index.md`. This is the script-side
 * twin of `markdownMirrorPath` in `src/content/seo.ts`; the two are pinned
 * together in `scripts/markdown-artifacts.test.mjs`.
 *
 * @param {string} pagePath
 */
export function markdownMirrorPath(pagePath) {
  if (pagePath === "/") return "/index.md";
  return `${pagePath.replace(/\/+$/, "")}.md`;
}

// Markdown artifacts that are not a mirror of a page: they document a boundary
// rather than restate a route. Hand-maintained, and the only ones that stay
// hand-maintained — everything else is generated by
// `scripts/markdown-mirrors.mjs` from `src/content/`.
export const STATIC_MARKDOWN_ARTIFACTS = [
  {
    path: "auth.md",
    route: "/auth.md",
    identity: "auth.md",
...
/**
 * The complete, finite list of Markdown artifacts allowed in the release.
 *
 * Derived rather than hand-listed: a new university or post adds a mirror, and
 * a manifest that had to be edited by hand would be the thing that goes stale.
 * `assertMarkdownRelease` compares this list against both `public/` and
 * `dist/`, so a mirror missing from either side fails the build.
 *
 * @param {Array<{path: string, route: string, identity: string, mediaType: string}>} mirrors
 */
export function buildMarkdownArtifacts(mirrors = []) {
  return [
    ...STATIC_MARKDOWN_ARTIFACTS,
    ...mirrors.map(({ path, route, identity, mediaType }) => ({
      path,
      route,
      identity,
      mediaType,
...
function assertNegativeUnsupportedClaim(value, label) {
  ...
  for (const match of value.matchAll(globalPattern)) {
    ...
    const context =
      `${value.slice(start, match.index)} ${value.slice(match.index + match[0].length, end)}` +
      answerFollowingQuestion(value, match.index);
...
/**
 * A FAQ heading asks «Является ли Vuzora официальным сервисом …?» and the
 * denial lives in the answer below it. The clause window cannot see that far,
 * so a question — and only a question, in a Markdown heading — is read
 * together with the paragraph that answers it. A statement still has to carry
 * its own negation.
 */
function answerFollowingQuestion(value, index) {
  const lineStart = value.lastIndexOf("\n", index) + 1;
  const lineEndIndex = value.indexOf("\n", index);
  const lineEnd = lineEndIndex === -1 ? value.length : lineEndIndex;
  const line = value.slice(lineStart, lineEnd);
  if (!/^#{1,6}\s/.test(line) || !line.trimEnd().endsWith("?")) return "";
  const rest = value.slice(lineEnd);
  const answer = rest.replace(/^\s*/, "").split(/\n\s*\n/)[0] ?? "";
  return ` ${answer}`;
}
