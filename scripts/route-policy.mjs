...
import { buildMarkdownArtifacts } from "./markdown-artifacts.mjs";
...
export function manifestFor({ universities, posts, mirrors = [] }) {
  return {
    universities: universities.map((record) => ({ ...record })),
    posts: [...posts],
    markdown: buildMarkdownArtifacts(mirrors),
  };
}
